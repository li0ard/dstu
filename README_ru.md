<p align="center">
    <b>@li0ard/dstu</b><br>
    <b>Алгоритмы ДСТУ на чистом TypeScript</b>
    <br>
    <a href="https://li0ard.is-cool.dev/dstu">документация</a>
    <br><br>
    <a href="https://github.com/li0ard/dstu/actions/workflows/test.yml"><img src="https://github.com/li0ard/dstu/actions/workflows/test.yml/badge.svg" /></a>
    <a href="https://github.com/li0ard/dstu/blob/main/LICENSE"><img src="https://img.shields.io/github/license/li0ard/dstu" /></a>
    <br>
    <a href="https://npmjs.com/package/@li0ard/dstu"><img src="https://img.shields.io/npm/v/@li0ard/dstu" /></a>
    <br>
    <hr>
</p>

## Установка

```bash
npm i @li0ard/dstu
```

## Поддерживаемые алгоритмы

- Эллиптические кривые и ЭП (ДСТУ 4145-2002)
    - Поддерживает полиномиальный и оптимальный нормальный базис
    - Поддерживает ECDH и шифрование
- Шифр "Калина" (ДСТУ 7624:2014)
- Хэш-функция "Купина" (ДСТУ 7564:2014)
    - Поддерживает KMAC
- Шифр "Струмок" (ДСТУ 8845:2019)
- Схема ECIES шифрования на скрученных кривых Эдвардса (ДСТУ 9041:2020)
- Шифр ДСТУ 9311:2024 (переизданный ДСТУ ГОСТ 28147:2009)
- Хэш-функция ГОСТ 34.311-95 на ДСТУ 9311:2024
    - Поддерживает HMAC

## Поддерживаемые режимы шифров

- Режим простой замены с зацеплением (CBC)
- Режим выработки имитовставки и гаммирования (CCM)
- Режим гаммирования с обратной связью по шифртексту (CFB)
- Режим гаммирования (CTR)
- Режим простой замены (ECB)
- Режим гаммирования с ускоренной выработкой имитовставки (GCM)
- Режим выработки имитовставки (MAC and GMAC)
- Режим гаммирования с обратной связью по выходу (OFB)
- Режим защиты ключа (KW)
    - Поддерживает [`Dstu7624Wrap`/`GOST28147Wrap`](https://zakon.rada.gov.ua/laws/show/z0108-13)
- Режим индексированной замены (XTS)

## Поддерживаемые кривые

| Битов | Полиномиальный базис<br>(ПБ) | Оптимальный нормальный<br> базис (ОНБ) | Тестовые |
|:----:|:------------------:|:-----------------------:|:----------:|
|  163 |         ✅         |                         |     ✅     |
|  167 |         ✅         |                         |            |
|  173 |         ✅         |           ✅            |  ✅ (ОНБ)  |
|  179 |         ✅         |           ✅            |            |
|  191 |         ✅         |           ✅            |            |
|  233 |         ✅         |           ✅            |            |
|  257 |         ✅         |                         |            |
|  307 |         ✅         |                         |            |
|  367 |         ✅         |                         |            |
|  431 |         ✅         |           ✅            |            |